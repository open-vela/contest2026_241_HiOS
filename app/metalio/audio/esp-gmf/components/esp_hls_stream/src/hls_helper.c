...
/* NuttX <nuttx/fs/fs.h> exposes its own internal file_open/read/seek/close
 * symbols with different signatures.  Rename the local static helpers via
 * macros so the upstream source compiles verbatim without collisions. */
#define file_open   hls_local_file_open
#define file_read   hls_local_file_read
#define file_seek   hls_local_file_seek
#define file_close  hls_local_file_close
