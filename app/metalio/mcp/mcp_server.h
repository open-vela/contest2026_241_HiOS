 ...
 *
 * MCP (Model Context Protocol) server — ported from
 * MetalioClaw4 main/mcp_server.h.
 *
 * Reference: https://modelcontextprotocol.io/specification/2024-11-05
 *
 * The MCP server:
 *   - Registers tools (functions callable from the AI assistant)
 *   - Parses JSON-RPC 2.0 requests from the connected protocol
 *   - Dispatches tool calls and returns results/errors
 *   - Optionally listens on a TCP port for external MCP clients
 *
 * Tool registration and dispatch logic is preserved verbatim from the
 * upstream implementation. Hardware-specific tools (camera, screen snapshot,
 * etc.) are conditionally compiled under HAVE_LVGL.
 */

#ifndef METALIO_MCP_SERVER_H
#define METALIO_MCP_SERVER_H
...
/* NOTE: Metalio is built with -fno-exceptions on NuttX to keep
 * firmware size down and avoid .gcc_except_table relocations that the
 * RISC-V toolchain cannot resolve for this target.  Replacing the
 * original `throw std::invalid_argument / runtime_error` calls with a
 * small helper macro that logs the message through ESP_LOGE (already
 * used throughout the file) and then calls abort() — the exact
 * equivalent of what GCC's -fno-exceptions throw lowering does, but
 * without needing exception-supporting library machinery. */
#ifndef MCP_FATAL
#  define MCP_FATAL(msg_)                                          \
      do {                                                         \
        ESP_LOGE("McpServer", "FATAL: %s", msg_);                  \
        ::std::abort();                                            \
      } while (0)
#  define MCP_FATAL_STR(str_)                                      \
      do {                                                         \
        const ::std::string& _s = (str_);                          \
        ESP_LOGE("McpServer", "FATAL: %s", _s.c_str());            \
        ::std::abort();                                            \
      } while (0)
#endif
#include <optional>
...
private:
    ...
    static std::string Base64Encode(const std::string &data)
    {
        ...
        mbedtls_base64_encode((unsigned char *)result.data(), result.size(),
                              &olen, (const unsigned char *)data.data(),
                              data.size());
        result.resize(olen);
        return result;
#else
        /* Minimal base64 encoder for builds without mbedTLS. */
...
public:
    /* Default constructor — required by std::vector<Property>::resize() /
     * _Construct_n() used internally by vector when it grows capacity
     * via copy-back operations (uClibc++ vector::insert uses the
     * uninitialized-fill path for default construction before
     * overwriting with actual elements).  Constructs a harmless empty
     * boolean property with has_default_value_=false so empty vectors
     * of Property can be built without triggering MCP_FATAL. */
    Property()
      : name_(""),
        type_(kPropertyTypeBoolean),
        value_(false),
          ...
          min_value_(min_value), max_value_(max_value)
    {
        if (type != kPropertyTypeInteger)
            MCP_FATAL("Range limits only apply to integer properties");
    }
          ...
          min_value_(min_value), max_value_(max_value)
    {
        ...
        if (default_value < min_value || default_value > max_value)
            MCP_FATAL("Default value must be within the specified range");
        value_ = default_value;
    }
    ...
    inline void set_value(const T &value)
    {
        if constexpr (std::is_same_v<T, int>)
        {
            if (min_value_.has_value() && value < min_value_.value())
                MCP_FATAL_STR("Value is below minimum allowed: " +
                              std::to_string(min_value_.value()));
            if (max_value_.has_value() && value > max_value_.value())
                MCP_FATAL_STR("Value exceeds maximum allowed: " +
...
public:
    ...
    PropertyList(std::initializer_list<Property> il)
      : properties_(il.begin(), il.end()) {}
    ...
    const Property &operator[](const std::string &name) const
    {
        ...
        MCP_FATAL_STR("Property not found: " + name);
        /* not reached */
        static Property dummy{"__dead__", kPropertyTypeBoolean, false};
        return dummy;
    }
...
public:
    ...
    /* Optional: start a TCP server for external MCP clients. */
    int StartTcpServer(uint16_t port = 2024);
    void StopTcpServer();
...
private:
    ...
    void TcpServerLoop(uint16_t port);
