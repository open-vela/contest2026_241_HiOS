 ...
 *
 * GpioLed implementation — ported from MetalioClaw4 main/led/gpio_led.cc.
 *
 * Only the LEDC driver include is swapped (ESP-IDF driver/ledc.h →
 * nuttx_ledc.h).  The state machine, blink timer and fade-callback logic
 * are preserved verbatim so the user-visible LED behaviour matches the
 * upstream firmware.
 */
                 ...
                 ledc_timer_t timer_num, ledc_channel_t channel)
{
    ...
    /* Prepare and set configuration of timers that will be used by LEDC. */
    ...
    /* When the fade callback fires, run led->OnFadeEnd(). */
    ledc_cbs_t ledc_callbacks = {};
    ledc_callbacks.fade_cb = FadeCallback;
    ledc_cb_register(ledc_channel_.speed_mode, ledc_channel_.channel,
                     &ledc_callbacks, this);

    esp_timer_create_args_t blink_timer_args = {};
    blink_timer_args.callback = [](void *arg) {
        auto led = static_cast<GpioLed *>(arg);
        led->OnBlinkTimer();
    };
    blink_timer_args.arg = this;
    blink_timer_args.name = "Blink Timer";
...
void GpioLed::StartBlinkTask(int times, int interval_ms)
{
    ...
    esp_timer_start_periodic(blink_timer_, (uint64_t)interval_ms * 1000);
}
