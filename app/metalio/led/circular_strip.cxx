 ...
 *
 * CircularStrip implementation — ported from MetalioClaw4
 * main/led/circular_strip.cc.
 *
 * Only the LED-strip driver include is swapped (ESP-IDF led_strip.h →
 * nuttx_led_strip.h).  The state machine, animation callbacks and
 * strip_timer logic are preserved verbatim so the user-visible LED ring
 * behaviour matches the upstream firmware.
 */
    ...
    : max_leds_(max_leds)
{
    ...
    strip_timer_args.callback = [](void *arg) {
    ...
    strip_timer_args.arg = this;
    strip_timer_args.name = "strip_timer";
...
void CircularStrip::FadeOut(int interval_ms)
{
    StartStripTask(interval_ms, [this]() {
        ...
        for (int i = 0; i < max_leds_; i++)
        {
            ...
            colors_[i].green /= 2;
            colors_[i].blue /= 2;
            if (colors_[i].red != 0 || colors_[i].green != 0 ||
                colors_[i].blue != 0)
                all_off = false;
            led_strip_set_pixel(led_strip_, i,
                                colors_[i].red, colors_[i].green,
    ...
    StartStripTask(interval_ms, [this, low, high]() {
        ...
        if (increase)
        {
            ...
            if (color.blue < high.blue)
                color.blue++;
            if (color.red == high.red && color.green == high.green &&
                color.blue == high.blue)
                increase = false;
        }
        else
        {
            ...
            if (color.blue > low.blue)
                color.blue--;
            if (color.red == low.red && color.green == low.green &&
                color.blue == low.blue)
                increase = true;
        }
        for (int i = 0; i < max_leds_; i++)
...
void CircularStrip::Scroll(StripColor low, StripColor high, int length,
                           int interval_ms)
{
    for (int i = 0; i < max_leds_; i++)
        colors_[i] = low;
...
void CircularStrip::StartStripTask(int interval_ms, std::function<void()> cb)
{
    ...
    strip_callback_ = std::move(cb);
    esp_timer_start_periodic(strip_timer_, (uint64_t)interval_ms * 1000);
}

void CircularStrip::SetBrightness(uint8_t default_brightness,
...
void CircularStrip::OnStateChanged()
{
    ...
    switch (device_state)
    {
        case kDeviceStateStarting:
        {
            ...
            StripColor high = {low_brightness_, low_brightness_,
                               default_brightness_};
        ...
        {
            StripColor color = {low_brightness_, low_brightness_,
                                default_brightness_};
            Blink(color, 500);
            break;
        }
        ...
        {
            StripColor color = {default_brightness_, low_brightness_,
                                low_brightness_};
            SetAllColor(color);
            break;
        }
