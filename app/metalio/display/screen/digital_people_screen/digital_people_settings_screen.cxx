 ...
 * main/display/screen/digital_people_screen/digital_people_settings_screen.cc.
 *
 * Adaptations for NuttX/openvela:
 *   - esp_log.h -> esp_log_shim.h
 *   - DigitalPeopleScreen::Create() -> DigitalPeopleScreen::CreateStatic()
 *      (so the back button uses the Screen-base-class wrapper).
 *   - pwr_key_handler.h already ported.
 *
 * The LVGL widget tree, layout constants, colour palette, format-card
 * selection logic, delay slider styling and lifecycle wiring are
...
void GoBackToDigitalPeople() {
    ...
    lv_obj_t* dp = DigitalPeopleScreen::CreateStatic();
...
lv_obj_t* DigitalPeopleSettingsScreen::CreateStatic() {
    s_ui = UiState{};
