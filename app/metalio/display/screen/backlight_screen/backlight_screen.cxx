 ...
 * main/display/screen/backlight_screen/backlight_screen.cc.
 */
...
int ReadInitialBrightness()
{
    ...
    if (bl != nullptr)
    {
        int cur = static_cast<int>(bl->GetBrightness());
        if (cur >= kBacklightMinPercent && cur <= 100)
            return cur;
    }
    Settings settings("display");
    int value = settings.GetInt("brightness", kBacklightDefaultPercent);
...
void UpdatePctLabel(int pct)
{
    ...
    if (pct < kBacklightMinPercent) pct = kBacklightMinPercent;
...
void ApplyBrightness(int value)
{
    ...
    auto *bl = Board::GetInstance().GetBacklight();
    if (bl != nullptr)
        bl->SetBrightness(static_cast<uint8_t>(value), true);
    else
        ESP_LOGW(TAG, "ApplyBrightness: no backlight driver");
}
...
lv_obj_t *CreateSlider(lv_obj_t *parent, int initial_brightness)
{
    ...
    lv_obj_add_event_cb(slider, OnSliderChanged, LV_EVENT_VALUE_CHANGED,
                        nullptr);
...
lv_obj_t *BacklightScreen::CreateStatic()
{
    ...
    lv_obj_set_style_text_color(slider_lbl, lv_color_hex(0xFFFFFF),
                                LV_PART_MAIN);
    ...
    lv_obj_set_style_text_color(range_lbl, lv_color_hex(0x9CA3AF),
                                LV_PART_MAIN);
