 ...
 * main/display/screen/translate_screen/translate_screen.cc.
 *
 ...
 *                                SetReceiveBufferSize / OnError are not
 *                                provided by the NuttX WebSocket and are
 *                                dropped with TODO comments.
 *   - HomeScreen::Create()    -> HomeScreen::CreateStatic().
 *   - vTaskDelay(pdMS_TO_TICKS(ms)) -> vTaskDelayMs(ms).
 *
 * The LVGL widget tree, layout constants, colour palette, language
 * table, Sonicloud protocol state machine, JSON parsing and audio
 * streaming loop are preserved verbatim from the original.
 */
...
void close_websocket() {
    ...
    if (s_ws) {
        // NuttX WebSocket exposes Stop() rather than Close(); the
        // destructor called by reset() also tears down the Rx thread.
        s_ws->Stop();
        s_ws.reset();
    }
...
bool connect_websocket(const std::string& ws_url, uint32_t session,
                       std::string& err_out) {
    // openvela: WebSocket is a standalone BSD-socket client
    // (websocket_protocol.h). The reference uses
    ...
    // TODO: NuttX WebSocket does not expose SetReceiveBufferSize; the
    // default receive path uses a 4 KB stack buffer in RxLoop. Bump it
    // here when a configurable buffer API is added.
    ...
    // TODO: NuttX WebSocket does not expose OnError. Disconnections and
    // socket errors are reported through OnDisconnected; fatal socket
    // failures surface as Connect() returning false below.
...
bool send_pcm_frame(const int16_t* samples, size_t count) {
    ...
    return s_ws->Send(reinterpret_cast<const char *>(samples), bytes, true);
}
...
void stream_pcm_loop(uint32_t session) {
    ...
    (void)as;
    /* Original ESP-IDF used Board::GetAudioCodec() for channel count check;
     * NuttX AudioService captures mono already. */
    ...
    while (!s_stop_requested.load(std::memory_order_acquire) &&
        ...
        /* NuttX AudioService captures mono; skip stereo down-mix. */
...
lv_obj_t* TranslateScreen::CreateStatic() {
    s_session.fetch_add(1, std::memory_order_acq_rel);
