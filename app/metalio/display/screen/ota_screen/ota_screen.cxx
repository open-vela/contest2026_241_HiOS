 ...
 *   - esp_lv_adapter_refresh_now → (no-op; LVGL refreshes in its own task)
 *   - esp_log.h → esp_log_shim.h
 *   - esp_timer.h → esp_timer_shim.h
 *   - Lang::Strings::* → I18n::T() (runtime lookup by string ID)
 *   - HomeScreen::Create() kept (resolved at link time once home_screen is ported)
 *   - lv_async_call kept (available in LVGL)
 */
...
lv_obj_t *BuildScreen(const char *version_text)
{
    ...
    lv_label_set_text(title, I18n::T("OTA 升级"));
    ...
    lv_label_set_text(status_lbl, I18n::T("正在升级"));
    ...
    lv_label_set_text(hint, I18n::T("请勿关闭电源"));
...
void OtaScreen::SetStatusMessage(const char *message)
{
    ...
    lv_unlock();
    ESP_LOGI(TAG, "OTA status: %s", message);
}
...
void OtaScreen::Dismiss()
{
    ...
    /* Return to home screen — HomeScreen::Create() is resolved at link time
     * once the home_screen module is compiled in. If home_screen is not yet
     * linked, we just delete the OTA screen and fall back to the previous
     * active screen. */
...
lv_obj_t *OtaScreen::Create()
{
    root_ = BuildScreen(nullptr);
    s_ui.screen = root_;
    s_ui.active = true;
    s_start_time_us = esp_timer_get_time();
    return root_;
