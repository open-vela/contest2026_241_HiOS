 ...
 *
 * Adaptation: inherits Screen base class; HomeScreen::Create() navigation
 * kept (resolved at link time). All game logic preserved verbatim.
 */
...
#include <stdlib.h>
#include <string.h>
/* NuttX lacks a C math.h; use GCC builtins instead of <cmath>. */
// #include <cmath>
...
void board_event_cb(lv_event_t *e)
{
    ...
    else if (code == LV_EVENT_RELEASED && s_pressing)
    {
        ...
        if (__builtin_abs(dx) < th && __builtin_abs(dy) < th)
            return;
...
void build_board(lv_obj_t *parent)
{
    ...
    /* Board owns swipe-to-move; keep screen edge swipe-back separate. */
...
void AttachNavBack(lv_obj_t *scr)
{
    screen_attach_swipe_back(scr, OnGoHome);
}

} // namespace

lv_obj_t *Game2048Screen::CreateStatic()
{
...
lv_obj_t *Game2048Screen::Create()
{
    root_ = CreateStatic();
    return root_;
}
