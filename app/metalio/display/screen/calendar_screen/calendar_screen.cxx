 ...
 * main/display/screen/calendar_screen/calendar_screen.cc.
 *
 * Faithful 1:1 port. The original is pure LVGL + POSIX localtime_r with
 * no hardware dependencies, so the entire UI (header row with accent bar
 * and "今天 M/D 周X" badge, weekday row, 6x7 day-cell grid with weekend
 * coloring / today highlight / prev-next-month greyed overflow, gradient
 * background) and all date math (days_in_month with leap-year handling,
 * first_weekday_of_month via mktime) are preserved verbatim.
 *
 * The only changes vs. the ESP-IDF source:
 *
 *   - `CalendarScreen::Create()` is split into `CreateStatic()` (the
 *     original factory) and `Create()` (the Screen base class wrapper).
...
void OnSwipeBack()
{
    /* Swipe-back is already deferred by screen_attach_swipe_back
     * (lv_async_call). SwitchToHome creates Home first, then deletes
     * calendar — sync delete-from-gesture previously solid-blued LVGL. */
    write(1, "CAL_BACK\n", 9);
...
lv_obj_t* CalendarScreen::CreateStatic() {
